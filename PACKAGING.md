...
Android 默认目标是 APK。APK 会保持旧方式，按 ABI 拆分产物。

打 official 正式 APK：

```bash
dart setup.dart android --env stable --targets=apk --channels=official
```

打所有渠道正式 APK：

```bash
dart setup.dart android --env stable --targets=apk
```

常见输出示例：

```text
dist/HawkVPN-0.0.6-android-official-release-arm64-v8a.apk
dist/HawkVPN-0.0.6-android-official-release-armeabi-v7a.apk
dist/HawkVPN-0.0.6-android-official-release-x86_64.apk
```

## Play AAB 打包

Google Play 专用 AAB 使用 `googleplay` 渠道。

要求支持 16KB 内存页面大小时，使用 arm64 单 ABI 构建：

```bash
dart setup.dart android --env stable --targets=aab --channels=googleplay --arch=arm64
```
...
```text
dist/HawkVPN-0.0.6-android-googleplay-release.aab
```

说明：

- `--arch=arm64` 会构建 `android-arm64`，并只保留 `arm64-v8a` native libs。
...
如果未指定 `--arch`，AAB 可能包含多个 ABI。当前 Play 16KB 要求建议使用上一节的 `--arch=arm64` 命令。
