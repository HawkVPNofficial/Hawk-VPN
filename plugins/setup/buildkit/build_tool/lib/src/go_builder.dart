...
class GoBuilder {
  ...
  String _goLdflags(Target target) {
    const androidPageSizeFlag = '-extldflags=-Wl,-z,max-page-size=16384';
    if (!target.isLib || target.goos != 'android') {
      return config.goLdflags;
    }
    if (config.goLdflags.contains('max-page-size=16384')) {
      return config.goLdflags;
    }
    return '${config.goLdflags} $androidPageSizeFlag';
  }
